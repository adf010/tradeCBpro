...
#read -p "Enter pagination cursor UUID : " page0

while true; do

BODY="${retail_portfolio_id}${eq1}${retail_portfolio_id1}${amps}${limit}${eq1}${limit0}${amps}${cursor}${eq1}${page0}"
...
(curl -L -s "${BENDPOINT}${requestpath}${qmark}${BODY}"  \
  ...
  --header "CB-VERSION: 2017-12-01" | jq . > CB-output.json )
  $editor CB-output.json
 
 page0=$(cat CB-output.json | grep -w cursor | sed 's/,*$//g' | sed 's/cursor://' | tr -d " ")
     echo $page0  
#echo -e "Would you like to see another page? "
...
case $yn in
       ...
       page0=$(cat CB-output.json | grep -w cursor | tr -d '"' | sed 's/,*$//g' | sed 's/cursor://' | tr -d " ")
       echo $page0
       page2=$page0
       read -p "Enter pagination cursor UUID (Or ENTER to continue): " page0
       if [[ $page0 == "" ]]; then
       page0=$page2
       fi
       echo -e '\E[32;40m'"\033[1m"
...
(curl -L -s "${BENDPOINT}${requestpath}${BODY}" \
 ...
 --header "CB-ACCESS-SIGN: $SIG" \
 --header "CB-ACCESS-TIMESTAMP: $TIMESTAMP" \
 --header "CB-VERSION: 2017-12-01" | jq -r . > CB-output.json )
 $editor CB-output.json
...
# curl -L 'https://api.coinbase.com/api/v3/brokerage/product_book?product_id=btc-usd&limit=4&aggregation_price_increment=.5' \
...
columns=$(tput cols)
lines=$(tput lines)
fold  -w "$columns" -bs  list_products.txt
echo
...
######################################################################################
 ...
 #########################################################################################
  ...
  columns=$(tput cols)
  lines=$(tput lines)
  fold  -w "$columns" -bs  get_product.txt
...
echo "Format start time as 12/01/2017 hh:mm"
read -p "Enter start time : " start1
start1=$(date -u -d "$start1" +%s)
echo "Format end time as 12/01/2024 hh:mm"
read -p "Enter end time : " end1
end1=$(date -u -d "$end1" +%s)
read -p "Select Granularity : " granularity1

requestpath="/api/v3/brokerage/products/${product_id1^^}/candles"

BODY="${qmark}${start0}${eq1}${start1}${amps}${end0}${eq1}${end1}${amps}${granularity}${eq1}${granularity1}"
...
read -p "Enter limit  * : " limit1
echo "Format start time as 12/01/2024 hh:mm"
read -p "Enter start (UNIX ) : " start1
echo "Format end time as 12/01/2024 hh:mm"
read -p "Enter end (UNIX ) : " end1
start1=$(date -u -d "$start1" +%s)
end1=$(date -u -d "$end1" +%s)
...
#(curl -L "${BENDPOINT}${requestpath}?limit=10&start=1712233072&end=1712333072" \
(curl -L "${BENDPOINT}${requestpath}${BODY}" \
 -X ${method}  \
 -H 'Content-Type: application/json' \
 --header "CB-ACCESS-KEY: $COINBASE_KEY" \
...
###################################################################################
    ...
    7)
 ##################################################################################
 #                      NOT DONE YET  TODO: ADD INPUT FOR order_id,  price  & size
 ##################################################################################
 # POST ORDER EDIT
 ...
 # --data-raw '{"order_id":"1","price":"5","size":"1"}'
 ...
 TIMESTAMP=$(date +%s)
 SIG=$(echo -n "${TIMESTAMP}POST/api/v3/brokerage/" | openssl dgst -sha256 -hmac "$COINBASE_SECRET" |cut -d' ' -f2); 
 read -p " " n
( curl -L -X POST 'https://api.coinbase.com/api/v3/brokerage/orders/edit' \
 -H 'Content-Type: application/json' \
 --header "CB-ACCESS-KEY: $COINBASE_KEY" \
 ...
 --data-raw '{"order_id":"1","price":"5","size":"1"}' | jq -r . > CB-output.json )
 $editor CB-output.json
...
###################################################################################
    ...
    8)
#########################################################################################
#   NOT FINISHED   ############                                      NOT DONE YET
#########################################################################################
# GET LIST ORDER HISTORY
# GET https://api.coinbase.com/api/v3/brokerage/orders/historical/batch
# curl -L -X GET 'https://api.coinbase.com/api/v3/brokerage/orders/historical/batch?product_id=0&user_native_currency=USD' \
...
read -p "Press ENTER to continue : " n
 ...
 TIMESTAMP=$(date +%s)
 SIG=$(echo -n "${TIMESTAMP}GET/api/v3/brokerage/orders/historical/batch" | openssl dgst -sha256 -hmac "$COINBASE_SECRET" |cut -d' ' -f2); 
 
( curl -L -s -X GET 'https://api.coinbase.com/api/v3/brokerage/orders/historical/batch?product_id=0&user_native_currency=USD' \
 -H 'Content-Type: application/json' \
 --header "CB-ACCESS-KEY: $COINBASE_KEY" \
 --header "CB-ACCESS-SIGN: $SIG" \
 --header "CB-ACCESS-TIMESTAMP: $TIMESTAMP" \
...
requestpath=""
 ...
 TIMESTAMP=$(date +%s)
 SIG=$(echo -n "${TIMESTAMP}${method}/api/v3/brokerage/" | openssl dgst -sha256 -hmac "$COINBASE_SECRET" |cut -d' ' -f2); 

( curl -L "${BENDPOINT}/api/v3/brokerage/" \
 -X ${method}  \
 -H 'Content-Type: application/json' \
 --header "CB-ACCESS-KEY: $COINBASE_KEY" \
 --header "CB-ACCESS-SIGN: $SIG" \
...
################
  ...
  product_id=${product_id^^}
  echo 
  read -p "Enter SELL, UNKNOWN_ORDER_SIDE if you do not want default [BUY]:" side0 ; side0=${side0:-BUY}
  side0=${side0^^}
...
read -p "Enter Product ID (ex. BTC-USD): " ids2
ids2=${ids2^^}
method="GET"
 requestpath="/api/v3/brokerage/orders/historical/fills"
 BODY="{${qmark}"${product_id}"${eq1}"${ids2}"}"
 echo "${BODY}"
   read -p "Press ENTER to continue: " n
 TIMESTAMP=$(date +%s)
 SIG=$(echo -n "${TIMESTAMP}${method}${requestpath}" | openssl dgst -sha256 -hmac "$COINBASE_SECRET" |cut -d' ' -f2);
...
method="GET"
 requestpath="/api/v3/brokerage/orders/historical/id"
 BODY="'{\"order_ids\":"${ids2}"}'"
 echo "${BODY}"
   read -p " " n
 TIMESTAMP=$(date +%s)
 SIG=$(echo -n "${TIMESTAMP}${method}/api/v3/brokerage/orders/historical/id" | openssl dgst -sha256 -hmac "$COINBASE_SECRET" |cut -d' ' -f2);

 (curl -L -X GET 'https://api.coinbase.com/api/v3/brokerage/orders/historical/id' \
 -H 'Content-Type: application/json' \
 --header "CB-ACCESS-KEY: $COINBASE_KEY" \
 --header "CB-ACCESS-SIGN: $SIG" \
 --header "CB-ACCESS-TIMESTAMP: $TIMESTAMP" \
...
# List deposits
#
# https://docs.cdp.coinbase.com/sign-in-with-coinbase/docs/api-deposits#list-deposits
#################################################################################
clear
columns=$(tput cols)
lines=$(tput lines)
...
#https://api.coinbase.com/v2/accounts/:account_id/deposits/:deposit_id

read -p "Enter Receive Account UUID: " payment_method1
requestpath="/v2/accounts/${payment_method1}/deposits"
...
(curl -L "https://api.coinbase.com/v2/accounts/${payment_method1}/deposits" \
  -X ${method} \
  -H 'Content-Type: application/json' \
  --header "CB-ACCESS-KEY: $COINBASE_KEY" \
  --header "CB-ACCESS-SIGN: $SIG" \
...
read -p "Press ENTER to close window. " y
 ...
 #############################################################
  ...
  read -p "Press ENTER to continue:" z
    ...
    4)
########################################################################################
# POST Wallet API: Deposits   v2
# POST https://api.exchange.coinbase.com/deposits/payment-method
...
#  }'
# https://docs.cdp.coinbase.com/sign-in-with-coinbase/docs/api-deposits#deposit-funds
##########################################################################################
# POST https://api.coinbase.com/v2/accounts/:account_id/deposits/    :deposit_id/commit
##########################################################################################
# GET https://api.coinbase.com/v2/accounts/:account_id/deposits   # List Deposits
##########################################################################################
# https://api.coinbase.com/v2/accounts/:account_id/deposits/    :deposit_id  # Show Deposit
##########################################################################################
clear
columns=$(tput cols)
lines=$(tput lines)
...
echo
 ...
 account_id1=""
 echo
 read -p "Enter Payment Method : " payment_method1
 read -p "Enter Receive Account : " account_id1
 read -p "Enter Amount : " amount1
 read -p "Enter Currency : " currency1

 requestpath="/v2/accounts/${account_id1}/deposits"
#
 BODY='{"amount":"'${amount1}'","currency":"'${currency1}'","payment_method":"'${payment_method1}'"}'
 echo "${BODY}"
 read -p "Recheck your entries. Then hit ENTER to execute or <CTRL>=c to abort. " n
...
( curl -L -X POST 'https://api.coinbase.com/api/v3/brokerage/' \
 -H 'Content-Type: application/json' \
 --header "CB-ACCESS-KEY: $COINBASE_KEY" \
 --header "CB-ACCESS-SIGN: $SIG" \
 --header "CB-ACCESS-TIMESTAMP: $TIMESTAMP" \
...
echo "Live Ticker                 = 8"
echo
read -p "Type a number [0, 1, 2, 3, 4, 5, 6, 7, 8] :" a
...
# BUY quote_size
  ...
  BODY='{"client_order_id":"${UUID1}","product_id":"ACH-USD","side":"SELL","order_configuration":{"market_market_ioc":{"base_size":"1"}}}'
 echo $body1
 echo $BODY
 read -p " stop5 orders " n


TIMESTAMP=$(date +%s)
SIG=$(echo -n "${TIMESTAMP}${method}${requestpath}${BODY}" | openssl dgst -sha256 -hmac "$COINBASE_SECRET" |cut -d' ' -f2);
SIG2=$(echo -n "${TIMESTAMP}${method}${requestpath}${BODY}}")
